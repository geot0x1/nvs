...
static int nvs_bitmap_set(uint32_t page_idx, uint16_t entry_idx, uint8_t bits)
{
    ...
    // printf("    NVS-Log: Bitmap P%u E%u set to %u (addr=0x%08X)\n", page_idx, entry_idx, bits, addr);
    return NVS_OK;
}
...
static int nvs_page_mark_full(uint32_t page_idx)
{
    ...
    nvs_page_hdr_t hdr;
    if (g_config.read(g_pages[page_idx].base_addr, &hdr, sizeof(hdr)) != 0) { return NVS_ERR_FLASH; }
        ...
        else if (bits == NVS_BITMAP_WRITTEN)
        {
            ...
            if (entry.span >= 1u)
            {
                ...
                else
                {
                    ...
                    // printf("    NVS-Scan: Found key '%s' (ns=%u, type=%u) on page %u\n", 
                    //        entry.key, entry.ns_index, entry.type, page_idx);
                    // fflush(stdout);
...
static int nvs_garbage_collect(void)
{
    ...
    for (uint32_t i = 0; i < g_num_pages; i++)
    {
        if ((int32_t)i == g_active_page) { continue; }
    ...
    {
        uint8_t bits;
        if (nvs_bitmap_get((uint32_t)victim, i, &bits) != NVS_OK) { continue; }
        if (bits != NVS_BITMAP_WRITTEN) { continue; }

        uint32_t src_addr = nvs_page_entry_addr((uint32_t)victim, i);
        nvs_entry_hdr_t hdr;
        if (nvs_read_entry_hdr(src_addr, &hdr) != NVS_OK) { continue; }
        ...
        // printf("    NVS-GC: Found valid entry '%s' E%u at 0x%08X (span=%u)\n", 
        //        hdr.key, i, src_addr, hdr.span);
        // fflush(stdout);
    ...
    for (uint32_t i = 0; i < gc_count; i++)
    {
        if (gc_buf[i].hdr.ns_index == 0u)
        {
            ...
            if (ns_idx > g_ns_count) { g_ns_count = ns_idx; }
        }
    }
        ...
        if (flash_addr >= g_pages[p].base_addr && flash_addr < page_end)
        {
            ...
            for (uint8_t s = 0; s < span; s++)
            {
                int rc = nvs_bitmap_set(p, (uint16_t)(entry_idx + s),
                                        NVS_BITMAP_ERASED);
                if (rc != NVS_OK) { return rc; }
                if (g_pages[p].written_count > 0) { g_pages[p].written_count--; }
                           ...
                           uint8_t type, const void *value, size_t vlen)
{
    ...
    if (handle == 0 || handle > NVS_MAX_HANDLES) { return NVS_ERR_HANDLE;      }
    ...
    if (!hr->in_use)   { return NVS_ERR_HANDLE;   }
                           ...
                           uint8_t type, void *out, size_t vlen)
{
    ...
    nvs_entry_hdr_t hdr;
    if (nvs_read_entry_hdr(e->flash_addr, &hdr) != NVS_OK) { return NVS_ERR_FLASH; }
    if (nvs_entry_crc(&hdr) != hdr.crc32)       { return NVS_ERR_CORRUPT; }
...
{
    if (config == NULL) { return NVS_ERR_INVALID_ARG; }
    ...
    if (config->num_sectors > 64u) { return NVS_ERR_INVALID_ARG; }
    ...
    for (uint32_t i = 0; i < g_num_pages; i++) { sorted_indices[i] = i; }
...
int nvs_open(const char *name, nvs_open_mode_t mode, nvs_handle_t *out_handle)
{
    ...
    if (ns_idx == 0)
    {
        if (mode == NVS_READONLY) { return NVS_ERR_NOT_FOUND; }
        if (g_ns_count >= NVS_MAX_NAMESPACES) { return NVS_ERR_NS_FULL; }
...
int nvs_commit(nvs_handle_t handle)
{
    if (!g_initialized) { return NVS_ERR_NOT_INIT; }
    if (handle == 0 || handle > NVS_MAX_HANDLES) { return NVS_ERR_HANDLE; }
    if (!g_handles[handle - 1u].in_use) { return NVS_ERR_HANDLE; }
    return NVS_OK;
}

void nvs_close(nvs_handle_t handle)
{
    if (handle > 0 && handle <= NVS_MAX_HANDLES) { g_handles[handle - 1u].in_use = 0; }
}

#define NVS_IMPL_SET_GET(SUFFIX, CTYPE, TYPE_CODE) \
int nvs_set_##SUFFIX(nvs_handle_t h, const char *k, CTYPE v) { return nvs_set_scalar(h, k, TYPE_CODE, &v, sizeof(CTYPE)); } \
int nvs_get_##SUFFIX(nvs_handle_t h, const char *k, CTYPE *out) { return nvs_get_scalar(h, k, TYPE_CODE, out, sizeof(CTYPE)); }
...
int nvs_set_str(nvs_handle_t handle, const char *key, const char *value)
{
    if (!g_initialized) { return NVS_ERR_NOT_INIT; }
    nvs_handle_rec_t *hr = &g_handles[handle - 1u];
    if (!hr->in_use || hr->mode != NVS_READWRITE) { return NVS_ERR_HANDLE; }
    ...
    if (total_span > 255u) { return NVS_ERR_VALUE_TOO_LARGE; }
...
int nvs_get_str(nvs_handle_t handle, const char *key, char *buf, size_t *len)
{
    ...
    nvs_lookup_t *e = NULL;
    if (nvs_lookup_find(hr->ns_index, key, &e) != NVS_OK || e->type != NVS_TYPE_STR) { return NVS_ERR_NOT_FOUND; }
    ...
    if (buf == NULL) { *len = total_size; return NVS_OK; }
    if (*len < total_size) { return NVS_ERR_BUF_TOO_SMALL; }
...
int nvs_erase_key(nvs_handle_t handle, const char *key)
{
    ...
    nvs_lookup_t *e = NULL;
    if (nvs_lookup_find(hr->ns_index, key, &e) != NVS_OK) { return NVS_ERR_NOT_FOUND; }
