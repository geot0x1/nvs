...
static int      read_sector_hdr(uint32_t base, uint32_t *magic, uint32_t *seq, uint32_t *state);
...
static uint32_t compute_entry_crc(uint8_t key_len, uint8_t data_len, const uint8_t *key, const uint8_t *data);
...
static uint32_t write_entry_to_active(uint8_t key_len, uint8_t data_len, const uint8_t *key, const uint8_t *data);
static void     get_sectors_by_seq_desc(uint8_t *out_indices, uint8_t *out_count);

static int      invalidate_visitor(uint32_t base, uint32_t off, uint8_t st, uint8_t kl, uint8_t dl, uint32_t crc, void *ctx);
static int      find_last_visitor(uint32_t base, uint32_t off, uint8_t st, uint8_t kl, uint8_t dl, uint32_t crc, void *ctx);
static int      gc_resume_visitor(uint32_t base, uint32_t off, uint8_t st, uint8_t kl, uint8_t dl, uint32_t crc, void *ctx);
static int      newer_copy_visitor(uint32_t base, uint32_t off, uint8_t st, uint8_t kl, uint8_t dl, uint32_t crc, void *ctx);
...
static nvs_err_t find_key_data_len(const char *key, uint8_t key_len, uint8_t *out_data_len);
...
nvs_err_t nvs_mount(const nvs_flash_driver_t *driver)
{
    ...
    for (uint8_t i = 0; i < SECTOR_COUNT; i++)
    {
        ...
        {
            /* read_sector_hdr() returned 0: magic mismatch or bad CRC.
             * Peek at the first word to distinguish a corrupt sector
             * (magic present, CRC wrong) from blank flash (0xFFFFFFFF). */
            uint32_t first_word;
            DRV_READ(sector_addr(i), &first_word, sizeof(first_word));
            if (first_word == NVS_MAGIC_WORD)
            {
                g_nvs.corrupt_sector_count++;
            }
        }
    }
...
nvs_err_t nvs_read(const char *key, void *buf, uint8_t buf_size, uint8_t *out_len)
{
    ...
    uint8_t   data_len;
    nvs_err_t rc = find_key_data_len(key, key_len, &data_len);
...
nvs_err_t nvs_get_size(const char *key, uint8_t *out_size)
{
    ...
    NVS_LOCK();
    nvs_err_t rc = find_key_data_len(key, key_len, out_size);
    NVS_UNLOCK();
    return rc;
}
...
{
    if (out_stats == NULL || !nvs_is_mounted())
    {
        return NVS_ERR_INVALID_ARG;
    }
    ...
    out_stats->total_sectors   = SECTOR_COUNT;
    out_stats->corrupt_sectors = g_nvs.corrupt_sector_count;
    ...
    out_stats->active_sectors = active;
    out_stats->free_sectors   = free;
...
/*===========================================================================
 *  Static local functions — sector addressing
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — sector header I/O
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — entry I/O
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — sector scanning
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — visitor callbacks
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — garbage collection
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — sector activation
 *===========================================================================*/
...
/*===========================================================================
 *  Static local functions — key lookup helper
 *===========================================================================*/

/**
 * Locate the newest valid copy of a key across all sectors, verify its CRC,
 * and write its data length to *out_data_len.
 *
 * Shared by nvs_read() and nvs_get_size() to keep the locate+verify logic
 * in one place.
 *
 * Caller must hold the NVS lock.
 */
static nvs_err_t find_key_data_len(const char *key, uint8_t key_len,
